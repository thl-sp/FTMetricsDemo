    ...
    public void DrawCarbonByShiftChart()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < ResultSet.GetLength(0); i++)
            {
                ...
                Log.Info(LogicObject.BrowseName, $"Shift = '{ResultSet[i, 0]}' - Value = '{ResultSet[i, 1]}'");

                Project.Current.GetObject("Model/Shift").GetVariable("Data" + (i + 1)).Value = Convert.ToString(ResultSet[i, 0]);
                Project.Current.GetObject("Model/Value").GetVariable("Data" + (i + 1)).Value = Convert.ToString(ResultSet[i, 1]);
